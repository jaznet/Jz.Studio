export function moveElementToNewBrowserWindow(element: HTMLElement, params: {
    ...
    let styles = Array.from(document.head.querySelectorAll('link')).map(x => x.cloneNode());
  ...
  if (params.newWindowClosedCallback != undefined) {
    ...
    function backupSs(el: Element) {
        if (el.shadowRoot) {
          ...
          for (const e of Array.from(el.shadowRoot.querySelectorAll('*'))) {
            backupSs(e);
          }
    ...
    backupSs(element);
  for (const e of Array.from(element.querySelectorAll('*'))) {
    backupSs(e);
  }
