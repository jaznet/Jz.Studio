...
export class DockManager {
    ...
    public context: DockManagerContext|undefined;
  public dockWheel: DockWheel | undefined;
  public layoutEngine: DockLayoutEngine | undefined;
  public mouseMoveHandler: EventHandler | undefined;
  public touchMoveHandler: EventHandler | undefined;
    ...
    constructor(element: HTMLElement|null, config?: DockConfig) {
    ...
    _onDialogDragEnded(sender: Dialog, e:any) {
        if (this.mouseMoveHandler) {
            this.mouseMoveHandler.cancel();
            delete this.mouseMoveHandler;
        }
    ...
    _requestTabReorder(container: IDockContainer, e:any) {
        let node = this._findNodeFromContainer(container);
     ...
     */
    requestUndockToDialog(container: PanelContainer, event:any, dragOffset: Point) {
        let node = this._findNodeFromContainer(container);
        this.layoutEngine!.undock(node);
    ...
    */
    ...
    getNodeByElementId(id: string): DockNode |undefined|null{
        let stack = [];
        stack.push(this.context!.model.rootNode);
    ...
    getNodeByElement(element: Element): DockNode |null|undefined {
        let stack = [];
        stack.push(this.context!.model.rootNode);
  ...
  private _findNodeFromContainer(container: IDockContainer): DockNode | undefined | null {
        let stack = [];
        stack.push(this.context!.model.rootNode);
    ...
    notifyOnUnDock(dockNode: DockNode|undefined|null) {
        this._checkShowBackgroundContext();
        this.layoutEventListeners.forEach((listener) => {
    ...
    notifyOnActiveDocumentChange(panel: PanelContainer|null, oldActive: PanelContainer|null) {
        this.layoutEventListeners.forEach((listener) => {
